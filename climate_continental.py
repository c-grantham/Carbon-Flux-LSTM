# -*- coding: utf-8 -*-
"""climate_Continental.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1tqLJbnqUf925VbobRrxqh0VKGpzZ-7pR
"""
...
sites = access_files(data_dir, "Arctic")
...
arctic_sites = date_conv(all_sites)
print(arctic_sites.head())

arctic_sites = arctic_sites.reset_index(drop=True)
nan_summary = arctic_sites.isna().sum().sort_values(ascending=False)
...
inputs_cont = [col for col in arctic_sites.columns if col not in targets + inputs_onehot + ['name', 'date', 'climate_Zone']]

# Split the data FIRST (using the raw data)
split_1 = int(len(arctic_sites) * 0.7)
split_2 = int(len(arctic_sites) * 0.85)

train_data = arctic_sites.iloc[:split_1]
val_data = arctic_sites.iloc[split_1:split_2]
test_data = arctic_sites.iloc[split_2:]
